...
###____________________________________________________sales page_______________________________________________________
    ...
    if st.checkbox("📊Display Pivot Table"):
        if select_col in ['ProductName', 'ModelName', 'Region', 'Country', 'Continent',
            ...
            styled_html = pivot_df.to_html(classes='styled-table')
            ...
            custom_css = """
                ...
                .styled-table th, .styled-table td {
                    border: 2px solid #444;      /* Softer border for dark mode */
                    padding: 10px;
                    text-align: center;
                }
                .styled-table th {
...
##_________________________________________________________Return Page_________________________________________________    
    ...
    if st.checkbox("📊Display Pivot Table"):
        if select_col in ['ProductName', 'ModelName', 'Region', 'Country', 'Continent',
            ...
            if aggfunc == 'sum':
                pivot_df = pd.pivot_table(filtered_df, index=select_col, columns='Season', values=values_col, aggfunc='sum', fill_value=0)
            elif aggfunc == 'mean':
                pivot_df = pd.pivot_table(filtered_df, index=select_col, columns='Season', values=values_col, aggfunc='mean', fill_value=0)
            elif aggfunc == 'count':
                pivot_df = pd.pivot_table(filtered_df, index=select_col, columns='Season', values=values_col, aggfunc='count', fill_value=0)
            elif aggfunc == 'max':
                pivot_df = pd.pivot_table(filtered_df, index=select_col, columns='Season', values=values_col, aggfunc='max', fill_value=0)
            else:
                pivot_df = pd.pivot_table(filtered_df, index=select_col, columns='Season', values=values_col, aggfunc='min', fill_value=0)
